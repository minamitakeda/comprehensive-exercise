...
public class UserRepository {
	...
	public List<Mail> findByKey(String key) {
		String sql = "SELECT * FROM mails WHERE key=:key";
		SqlParameterSource param = new MapSqlParameterSource().addValue("key", key);
		List<Mail> userRegisterList2 = template.query(sql, param, USERREGISTER_ROW_MAPPER);
		if (userRegisterList2.size() == 0) {
			return null;
		}
		return userRegisterList2;
	}
	public List<Mail> findByEmail2(String key) {
		String sql = "SELECT * FROM mails WHERE key=:key AND date +cast('1 days' as interval)>now();";
		SqlParameterSource param = new MapSqlParameterSource().addValue("key", key);
